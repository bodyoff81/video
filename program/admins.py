...
@Client.on_message(command(["تخطي","skip","s", f"skip@{BOT_USERNAME}", "vskip"]) & other_filters)
@authorized_users_only
...
@Client.on_message(
    command(["وقف","stop","ايقاف", f"stop@{BOT_USERNAME}", "end", f"end@{BOT_USERNAME}", "vstop"])
    & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["ايقاف مؤقت", f"pause@{BOT_USERNAME}", "vpause"]) & other_filters
)
@authorized_users_only
